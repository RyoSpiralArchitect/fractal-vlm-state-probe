...
from fractal_vlm_state_probe.prompt_robustness_pair_matrix import (
    ...
    write_prompt_robustness_pair_matrix_json,
    write_prompt_robustness_pair_matrix_markdown,
)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare prompt robustness over a model x source-pair matrix."
    )
    parser.add_argument(
    ...
    analysis = analyze_prompt_robustness_pair_matrix(
        analyses,
        analysis_paths=paths,
    )
    write_prompt_robustness_pair_matrix_json(analysis, args.output_json)
    write_prompt_robustness_pair_matrix_markdown(analysis, args.output_md)
    print(f"wrote model x source-pair prompt matrix to {args.output_md}")
